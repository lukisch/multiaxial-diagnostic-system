...
\begin{tabular}{lcccc}
\toprule
Merkmal & OPCRIT & CATEGO & MHIRA & Vorliegendes Modell \\
\midrule
Multiaxiale Struktur & Nein & Nein & Nein & Ja (6 Achsen) \\
Duale Kodierung (DSM/ICD) & Partiell & Nur ICD & Flexibel & Vollst\"andig \\
Abdeckungsanalyse & Nein & Nein & Nein & Ja \\
Dimensionale Integration & Nein & Nein & Partiell & Ja (HiTOP) \\
FHIR-Interoperabilit\"at & Nein & Nein & Partiell & Geplant \\
Empirische Validierung & Ja & Ja & Partiell & Geplant \\
\bottomrule
\end{tabular}
\label{tab:vergleich}
\end{table}
...
\textbf{Quantitative Abdeckungsanalyse (Ii):} Die Abdeckungsanalyse wird durch eine Symptom-Diagnosen-Matrix operationalisiert, in der jedes Symptom einer oder mehreren erkl\"arenden Diagnosen zugeordnet wird. F\"ur jedes Symptom wird ein Abdeckungsprozentsatz berechnet, der den Grad der diagnostischen Erkl\"arung quantifiziert. Der Gesamtabdeckungswert (z.\,B.\ $\sim$88\%) gibt dem Kliniker eine quantitative R\"uckmeldung \"uber die Vollst\"andigkeit seiner diagnostischen Arbeit. Symptome werden als \textit{vollst\"andig abgedeckt} ($\geq$85\%), \textit{partiell abgedeckt} (60--84\%) oder \textit{unzureichend abgedeckt} ($<$60\%) klassifiziert.
...
Sei $S = \{s_1, \ldots, s_n\}$ die Menge der beobachteten Symptome und $D = \{d_1, \ldots, d_m\}$ die Menge der etablierten Diagnosen. F\"ur jedes Symptom $s_i$ definieren wir die Abdeckungsfunktion $c(s_i) = \max_{d_j \in D} w(s_i, d_j)$, wobei $w(s_i, d_j) \in [0, 1]$ das Gewicht des Symptoms $s_i$ in der Diagnose $d_j$ gem\"a\ss{} den diagnostischen Kriterien repr\"asentiert. Die Gesamtabdeckung ergibt sich als $C_{\text{total}} = \frac{1}{n} \sum_{i=1}^{n} c(s_i)$. Die Symptomgewichte werden aus den diagnostischen Kriterien von DSM-5-TR und ICD-11 abgeleitet, wobei jedes Kriterium innerhalb einer Diagnose gleich gewichtet wird. \"Uberlappende Diagnosen werden behandelt, indem jedes Symptom der Diagnose mit maximaler Abdeckung zugeordnet wird.

\subsection{Implementierung}

Die Implementierung arbeitet wie folgt: (1)~Sammlung aller best\"atigten Symptome \"uber alle Screening-Instrumente, (2)~Zuordnung jedes Symptoms zu den diagnostischen Kriterien, die es erf\"ullt, (3)~Berechnung eines Abdeckungsprozentsatzes pro Symptom, der den Grad der diagnostischen Erkl\"arung quantifiziert, (4)~Klassifikation als vollst\"andig abgedeckt ($\geq$85\%), partiell abgedeckt (60--84\%) oder unzureichend abgedeckt ($<$60\%), (5)~Berechnung einer Gesamtabdeckungswert als gewichteter Mittelwert \"uber alle Symptome, (6)~Pr\"asentation der Ergebnisse als Symptom-Diagnosen-Matrix mit visueller Hervorhebung diagnostischer L\"ucken. Das 4P-Modell (Achse~V) dient als nat\"urliches Komplement: Symptome, die nicht durch Achse-I-Diagnosen abgedeckt sind (Subachse~Ii), sollten durch die Achse-V-Formulierung erkl\"arbar sein; solche, die es nicht sind, repr\"asentieren genuine diagnostische L\"ucken und m\"unden in den priorisierten Untersuchungsplan (Subachse~Ij).

Die quantitative Metrik (z.\,B.\ \glqq Gesamtabdeckung: $\sim$88\%\grqq{}) liefert dem Kliniker eine unmittelbare R\"uckmeldung \"uber die Vollst\"andigkeit seiner diagnostischen Arbeit --- ein Feature, f\"ur das uns keine direkt vergleichbare Implementierung bekannt ist, obwohl verwandte Ans\"atze existieren.
...
Das vorgestellte 6-Achsen-Modell ist kein R\"uckgriff auf das DSM-IV, sondern repr\"asentiert eine \textbf{vorgeschlagene Weiterentwicklung}, auf die die psychiatrische Fachwelt selbst konvergiert. Neun vorgeschlagene Designmerkmale verdienen besondere Hervorhebung:

\textbf{Erstens} erweitert die Abdeckungsanalyse (Abschnitt~\ref{sec:abdeckung}) bestehende Entscheidungsunterst\"utzungsmethoden um eine transparente, prozentbasierte Metrik, f\"ur die uns keine direkt vergleichbare Implementierung in der publizierten psychiatrischen Informatik bekannt ist. Sie adressiert das von Nordgaard et al.\ (2016) dokumentierte Grundproblem transdiagnostischer Symptom\"uberlappung und wird symmetrisch in Achse~I (Ii) und Achse~III (IIIi) implementiert.
...
\textbf{Viertens} stellt die symmetrische Parallelstruktur von Achse~I und Achse~III sicher, dass das System als gemeinsames Werkzeug f\"ur alle beteiligten Professionen funktioniert --- ein Designprinzip, das nach unserem Kenntnisstand in bisherigen Systemen nicht verfolgt wurde.
...
\textbf{Empirische Kalibrierung der Abdeckungsanalyse erforderlich:} Die Abdeckungsanalyse --- f\"ur die uns keine direkt vergleichbaren Implementierungen bekannt sind, obwohl verwandte Ans\"atze existieren --- ist in Abschnitt~\ref{sec:abdeckung} formal definiert, aber noch nicht empirisch kalibriert. Die prozentbasierte Symptom-Diagnosen-Zuordnung ist intuitiv ansprechend, wirft jedoch ungel\"oste Fragen auf: Welche Abdeckungswerte sind in verschiedenen diagnostischen Populationen typisch? Wie sensitiv ist der Gesamtabdeckungswert gegen\"uber einzelnen Symptombewertungen? Die vorgeschlagenen Schwellenwerte (85\%/60\%) bed\"urfen empirischer Validierung durch die geplanten Feldstudien.
...
Das vorgeschlagene 6-Achsen-System adressiert eine strukturelle L\"ucke in der psychiatrischen Diagnostik, die seit der Abschaffung des DSM-IV-Multiaxialsystems besteht. F\"ur die klinische Praxis liegt der prim\"are Beitrag des Modells in der Wiederherstellung --- und substanziellen Erweiterung --- der strukturierten Aufforderung zur umfassenden biopsychosozialen Beurteilung. Die symmetrische Achse-I/III-Architektur erm\"oglicht genuine multi-professionelle Zusammenarbeit, die Abdeckungsanalyse liefert Klinikern eine transparente, quantitative Metrik f\"ur diagnostische Vollst\"andigkeit, und das CAVE-Warnsystem adressiert die wachsende Herausforderung des achsen\"ubergreifenden Risikomanagements bei komplexen F\"allen. Die PRO/CONTRA-Evidenzbewertung mit Konfidenzsch\"atzung operationalisiert diagnostische Transparenz in einer Weise, die sowohl klinische Rechenschaftspflicht als auch Ausbildung unterst\"utzt.

Die Architektur des Modells ist konsistent mit der Richtung, in die sich die gro\ss{}en Klassifikationssysteme entwickeln. Das APA Future DSM Strategic Committee exploriert Dimensionalit\"at, Biomarker und soziale Determinanten --- alles Aspekte, die das vorliegende Modell bereits durch HiTOP-Integration, RDoC-Annotationsf\"ahigkeit und die strukturierte Achse~IV ber\"ucksichtigt. Die Konvergenz mit der expliziten Forderung von \citet{ErlichFirst2025SocialDeterminants} nach einer R\"uckkehr zur strukturierten axialen Beurteilung unterst\"utzt die Aktualit\"at dieses Beitrags. Zugleich geht das Modell \"uber das hinaus, was institutionelle Revisionen allein wahrscheinlich erreichen werden, insbesondere durch die Abdeckungsanalyse und die operationalisierte Fallformulierung (Achse~V), die Klassifikation und Behandlungsplanung verbr\"uckt.

Konkrete n\"achste Schritte umfassen: (1)~die Durchf\"uhrung des Phase-1-Experten-Reviews zur Verfeinerung der Achsenstruktur, (2)~die Durchf\"uhrung der Interrater-Reliabilit\"atsstudie mit standardisierten Fallvignetten (Phase~2), (3)~die Entwicklung der HL7-FHIR-Mapping-Spezifikation f\"ur die Interoperabilit\"at mit bestehenden Gesundheits-IT-Systemen und (4)~die Publikation der formalen mathematischen Spezifikation der Abdeckungsanalyse mit klinischen Durchrechnungsbeispielen. Der vollst\"andige Quellcode ist \"offentlich verf\"ugbar, um unabh\"angige Evaluation und Replikation zu erm\"oglichen.
